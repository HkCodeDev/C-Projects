...
int inputIntPositive(void) {
	...
	while (!positiveInputReceived) {
		...
		char inputStr[MEMORY];  // Assuming a reasonable size for the input
		char intValueStr[MEMORY];
		...
		for (i = length - 1; i >= 0; i--) {
			intValueStr[length - i - 1] = (tempValue % 10) + '0';
			tempValue /= 10;
		}
		intValueStr[length] = '\0';

		// Copy the manually converted string to inputStr
		strcpy(inputStr, intValueStr);

		if (inputValue > 0 && strcmp(inputStr, "0") != 0) {
			// Set the flag to exit the loop
...
char inputCharOption(const char* character) {
	...
	while (!valid) {
		...
		while (i < strlen(character) && !found) {
			if (inputValue == character[i]) {
				found = 1;
			}
			i++;
		}
